...
export default function CodeSnippet({
  ...
  const { fontSize, theme, setTheme } = useCodeEditorStore();
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (err instanceof TypeError && err.message.includes("fetch")) {
        setError(
          "Network error: Unable to connect to the code execution service"
        );
  ...
  return (
    <div className="grid grid-cols-1 gap-2">
      <div className="relative">
        <div className="bg-[#12121a]/90 backdrop-blur rounded-md border border-white/[0.05] p-4">
          {/* Header */}
